...
$ret = wp_update_term( $tag_ID, $taxonomy, $_POST );
 $taxn=array("related_pa_sell","related_pa_rent","related_pa_floor","related_pa_filter-colour","related_pa_style","related_product_color","related_product_accents");

$eVal=explode(",",ltrim(",",$_POST['extId']));
/*   print_r($_POST['extId']);
 ...
 foreach($_POST as $pk => $pv)
{
	foreach($pv as $ppk => $ppv)
	{
		...
		echo $pk;
		echo "<br/>"; 
		if (!in_array($ppv, $eVal) && in_array($pk, $taxn))
  {
	  //echo $ppv;
$eVal[]=$ppv;
$k++;
...
$txnn=str_replace("related_","",$tx);
/* echo $ev.$txnn;
	echo "<br/>"; */
$naame[$i]=get_term_by('name',esc_attr($ev), $txnn,ARRAY_N);

if(!empty($naame[$i][0]))
{

wp_delete_term( $naame[$i][0], $txnn );

$i++;
}
$naame2[$i]=get_term_by('name',esc_attr($ev), $txnn,ARRAY_N);
/* print_r($naame2[$i]);
echo "<br/>"; */
}
}
...
$searchString = ',';
 ...
 if( strpos($_POST['selPro'], $searchString) !== false ) {
	 
 $proids=explode(",",$_POST['selPro']);
 
   foreach($proids as $prod )
{
$proId=$prod;
foreach($taxn as $tx)
{
...
$tx=str_replace("related_","",$tx);

$ter=get_term_by('id',$ev, $tx,ARRAY_N);
...
echo $tx;
echo "<br/>"; */
wp_set_object_terms( (int)$proId, $ides,$tx,false );
unset($ides);
}
} 
 }
...
echo $tx;
echo "<br/>"; */
$ab=wp_set_object_terms( (int)$proId, $ides,$tx,false );
/* print_r($ab); */
unset($ides);
}
