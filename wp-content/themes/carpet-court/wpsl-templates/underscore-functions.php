...
<script id="wpsl-info-window-template" type="text/template">
    <?php
        ...
        $info_window_template .= "\t\t" . '<p class="top-title">' . "\r\n";
        ...
        $info_window_template .= "\t\t" . '<%= createInfoWindowActions( id ) %>' . "\r\n"."<br><hr style='margin: 18px 0;'><strong style='font-size: 14px;'>Contact information:</strong><br>";

        $info_window_template .= "\t\t" . '<% if ( phone ) { %>' . "\r\n";
        $info_window_template .= "\t\t" . '<span style="padding: 8px 0 2px 0;"><strong>' . esc_html( $wpsl->i18n->get_translation( 'phone_label', __( 'Phone', 'wpsl' ) ) ) . '</strong>Phone: <%= formatPhoneNumber( phone ) %></span>' . "\r\n";
        $info_window_template .= "\t\t" . '<% } %>' . "\r\n";
        $info_window_template .= "\t\t" . '<% if ( fax ) { %>' . "\r\n";
        $info_window_template .= "\t\t" . '<span style="padding: 2px 0;"><strong>' . esc_html( $wpsl->i18n->get_translation( 'fax_label', __( 'Fax', 'wpsl' ) ) ) . '</strong>Fax: <%= fax %></span>' . "\r\n";
        $info_window_template .= "\t\t" . '<% } %>' . "\r\n";
        $info_window_template .= "\t\t" . '<% if ( email ) { %>' . "\r\n";
        $info_window_template .= "\t\t" . '<span style="padding: 2px 0; margin-bottom: 8px;"><strong>' . esc_html( $wpsl->i18n->get_translation( 'email_label', __( 'Email', 'wpsl' ) ) ) . '</strong><%= email %></span>' . "\r\n";
        $info_window_template .= "\t\t" . '<% } %>' . "\r\n";

        //link
        $info_window_template .= "\t\t\t" .  wpsl_link_template() . "\r\n";  // Check which header format we use
    ...
    if ( $wpsl_settings['permalinks'] ) {
        ...
        if ( $location == 'wpsl_map') {
            ...
            $header_template .= $tab . '<% } %>';
             ...
             $header_template .= '<% } %>';
             // $header_template .= '<span class="store-stat"><%=store_status%></span>';
        }
    } else {
 ...
 * @return string $header_template The template for the store header
 */
function wpsl_link_template() {

    global $wpsl_settings;

    if ( $wpsl_settings['permalinks'] ) {
            $header_template = '<strong><a href="<%= permalink %>">View More</a></strong><br/>';
    }
    return apply_filters( 'wpsl_link_template', $header_template );
}
...
function cc_get_star_rating($post_id) {
    $all_comments = get_comments( array('post_id' => $post_id ) );
    echo '<pre>';
    var_dump($post_id);
    // var_dump($all_comments[0]->comment_ID);
    echo '</pre>';
}
