 ...
 * @returns {string} zipcode or formatted address  The zipcode or the formatted address
 */
function filterApiResponse( response ) {
	...
	if (typeof zipcode !== 'undefined' && zipcode !== '') {
		...
		if (typeof response[0].formatted_address === 'string') {
			output = response[0].formatted_address;
		}
	}
...
function makeAjaxRequest( startLatLng, resetMap, autoLoad, infoWindow ) {
	...
	*/
	...
	$storeList.empty().append( "<li class='wpsl-preloader clearfix'> <div class='loader-wrap'> <div class='loader-center'> <img src='" + preloader + "'/>" + wpslLabels.preloader + "</div></div></li>" );
...
function addMarker( latLng, storeId, infoWindowData, draggable, infoWindow ) {
	...
	mapIcon = {
		url: url,
		scaledSize: new google.maps.Size( map_icon_width, map_icon_height ), //retina format
...
function radiusChanged() {
    var center = map.getCenter();
 	startLatLng = new google.maps.LatLng( center.lat(), center.lng() );

	makeAjaxRequestMapChanged( startLatLng, resetMap, false ,infoWindow);
	google.maps.event.trigger(map, "resize");

}


$(window).data('ajaxZoomReady','true');
$(window).data('ajaxDragReady','true');

google.maps.event.addListener(map, 'zoom_changed', function() {
	if( $(window).data('ajaxZoomReady') == 'true' ){
		$(window).data('ajaxZoomReady','false');
...
google.maps.event.addListener(map, 'dragend', function() {
	if( $(window).data('ajaxDragReady') == 'true' ){
		$(window).data('ajaxDragReady','false');
...
function makeAjaxRequestMapChanged( startLatLng, resetMap, autoLoad, infoWindow ) {
	var latLng, maxResult, radius,
		storeData  = "",
		categoryId = "",
		draggable  = true,
