...
<form id="yith-wcwl-form" action="<?php echo esc_url( YITH_WCWL()->get_wishlist_url( 'view' . ( $wishlist_meta['is_default'] != 1 ? '/' . $wishlist_meta['wishlist_token'] : '' ) ) ) ?>" method="post" class="woocommerce">
    ...
    <!-- WISHLIST TABLE -->
    <?php
    $wishlistss = YITH_WCWL()->get_wishlists( array( 'user_id' => get_current_user_id() ) );
    $prod_wishlist = array();
    global $wpdb;
    foreach ($wishlistss as $wishlists_value) {
       $prod_id_query = $wpdb->get_results( $wpdb->prepare( "SELECT prod_id, wishlist_id, wishlist_token from ".$wpdb->prefix."yith_wcwl as ycl INNER JOIN ".$wpdb->prefix."yith_wcwl_lists as ywl ON ycl.wishlist_id = ywl.ID where ycl.wishlist_id = %d", $wishlists_value['ID'] ), ARRAY_A );
       foreach ($prod_id_query as $prod_value) {
           array_push( $prod_wishlist, $prod_value );
       }
   }
        ...
        <tr>
           <?php if( $show_cb ) : ?>
            ...
            <?php if( $show_price ) : ?>
                ...
                <tbody>
                    <?php
                    if( count( $prod_wishlist ) > 0 ) :
                        foreach( $prod_wishlist as $item ) :
                            global $product;
                   ...
                   ?>
                   <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo ( is_user_logged_in() ) ? esc_attr( $item['wishlist_id'] ) : '' ?>" data-token="<?php echo ( ! empty( $item['wishlist_token'] ) && is_user_logged_in() ) ? esc_attr( $item['wishlist_token'] ) : '' ?>" class=" cart wishlist_table">
                       <?php if( $show_cb ) : ?>
                    ...
                    <td class="product-thumbnail here2">
                        <a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">
                            <?php
                            if( get_field('featured_image', $item['prod_id']) ){
                              echo '<img src="'.get_field('featured_image', $item['prod_id']).'" alt="Product Image" class="woocommerce-image wp-post-image" width="112" height="80">';
                              // echo '<img src="'.get_field('featured_image', $item['prod_id']).'">';
                            }else{
...
<?php if ( is_user_logged_in() ) {
    ...
    $result_arr = $wpdb->get_results('SELECT * from '.$table_name.' where user_id = '.$current_user_id );
    if ( !empty( $result_arr ) ) {

        ?>
        <div class="wishlist-title ">
            <h2>My Wishlist Images</h2>
        </div>

        <table class="shop_table">
            <thead>
                <tr>
                    <th class="product-thumbnail"><?php echo __( 'Image', 'carpet-court'); ?></th>
                    <th class="product-name">
                        <span class="nobr"><?php echo __( 'Action', 'yith-woocommerce-wishlist' ); ?></span>
                    </th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    <td>
                        <a href="<?php echo $result_value->media_url_large; ?>" rel="prettyPhoto">
                        <img class="wishlists-img" src="<?php echo $result_value->media_url_small; ?>">
                        </a>
                    </td>
                    <td>
                        <a href="#" class="remove_wishlist_img btn btn-cc-regular-red" data-rowid="<?php echo $result_value->id; ?>" title="Remove From Wishlists" data-mediaid="<?php echo $result_value->media_id; ?>">Remove</a>
                    </td>
                </tr>
                <?php
            }
